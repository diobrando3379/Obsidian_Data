...
| E1-fstgan_exp6_15 | `tcn3`        | BiMamba2_1D       | `(N,C,T,V) → (N*V,T,C)`               | 双向 Mamba 处理长程时序依赖，替换末段下采样时序路径，提升全局时序建模。【F:布尔大学士模块/B站更新（和视频同步更新）/046 BiMamba2_1D.py†L193-L229】                                                                                  |
| E2-fstgan_exp6_16 | `tcn2`        | MDM               | `(N,C,T,V) → (N*V,C,T)`               | 多尺度时间分解 + 线性混合，适合作为中段细粒度时间增强。【F:布尔大学士模块/B站更新（和视频同步更新）/101 Multi-Scale Decomposable Mixing Block（AAAI 2025）.py†L15-L75】                                                         |
